...
- **후속 백로그(미착수)**: ① `SUBJECT_OVERRIDES` 커버리지 공백 — 태그 21개 중 `education`·`humanities`·`law-politics`·`literature` 4개엔 전문교과가 하나도 안 붙음. 특히 예술계열의 문예창작·문학 계열 과목 다수와 국제계열의 `국제법`·`국제 정치`(현재 `global`만, `law-politics` 미부여)는 명백한 오버라이드 기회. 추천 결과가 바뀌므로 사용자 확인 후 진행할 것. ② 추천 페이지 통합 로직(dedup·개설 필터·안정 정렬·`professionalOffered` 게이팅)에 자동 테스트 없음 — 현재 Playwright 수동 검증에만 의존. ③ `AREA_TO_TAGS` 키의 `·`(U+00B7)/`･`(U+FF65) 혼용 — subjects.json 원본과 일치 확인됨, 다른 원본과 대조 시에만 주의.
