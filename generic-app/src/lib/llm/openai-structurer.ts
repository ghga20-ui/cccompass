...
export function createSystemPrompt() {
  return [
    ...
    // --- 집중이수 ↔ ---
    "SPLIT '↔' ACROSS SEMESTERS, NEVER KEEP AS ONE NAME. A name containing '↔' (e.g. '정보↔한문', '음악↔미술', '논술↔생태와 환경') denotes 집중이수/교차운영 (two subjects taught in alternating semesters). Place the FIRST subject in the earlier semester (1학기) requiredSubjects and the SECOND subject in the later semester (2학기) requiredSubjects — e.g. '정보↔한문' → 정보 in 1학기, 한문 in 2학기. Do NOT keep 'A↔B' as a single name and do NOT model it as a 택1 choiceGroup. If you cannot tell the semester order, you may still emit 'A↔B' and it will be split automatically (first→1학기, second→2학기).",
    // --- 지정 vs 선택군 ---
