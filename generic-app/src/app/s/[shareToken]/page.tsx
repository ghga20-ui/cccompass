...
export default function ShareHomePage() {
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      {/* 히어로 — 틴트존 + 컴퍼스 모티프 배경 */}
      <section className="relative overflow-hidden bg-[var(--secondary)] px-5 pb-8 pt-10 md:pt-16 md:text-center">
        <CompassMotif className="pointer-events-none absolute left-1/2 top-1/2 h-[440px] w-[440px] -translate-x-1/2 -translate-y-1/2 text-[var(--primary)] opacity-[0.06]" />
        ...
        <section className="px-5 pb-4 pt-5">
          <Reveal className="mx-auto max-w-lg">
            <div className="mb-2.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
              ...
              <h2 className="text-sm font-medium text-foreground">입학 연도를 선택해줘</h2>
              <span className="text-xs text-muted-foreground">학년마다 개설 과목이 달라</span>
            </div>
      ...
      {/* 관심 분야 — 메인 행동 구역(틴트 카드로 그룹핑) */}
      <section className="flex-1 px-5 pb-6 pt-2">
        <Reveal className="mx-auto max-w-lg">
          <div className="rounded-2xl border border-[var(--primary)]/10 bg-[var(--secondary)]/60 p-5">
      ...
      {/* CTA — 오렌지 틴트 마감 존 + 상태 안내 카피 */}
      <section className="sticky bottom-[72px] z-30 border-t border-[var(--cta)]/15 bg-orange-50/90 px-5 py-3 backdrop-blur-md md:bottom-0">
        <div className="mx-auto max-w-lg">
          <p
            className={cn(
              "mb-2 text-center text-xs font-medium",
              canProceed ? "text-[var(--cta)]" : "text-muted-foreground",
            )}
          >
            {canProceed
              ? "준비 완료! 맞춤 과목을 볼 수 있어"
