...
export default function SubjectDetailPage({
  ...
  const backHref = getInternalReturnPath(searchParams.get("from"), basePath);

  const schoolAvailability = useMemo((): SchoolAvailability[] => {
    if (!subject || !cohortData) return [];
    const results: SchoolAvailability[] = [];

    cohortData.designated.forEach((d) => {
      if (expandSubjectNames(d.subject).includes(subject.name)) {
        results.push({
          grade: d.grade,
          semester: d.semester,
          label: "지정 과목",
    ...
    cohortData.selections.forEach((group) => {
      if (
        group.options.some((name) =>
          expandSubjectNames(name).includes(subject.name),
        )
      ) {
        results.push({
          grade: group.grade,
          ...
          <Card className="border-border/60">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-foreground">관련 직업</h3>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {subject.relatedCareers.map((career) => (
                  <Badge key={career} variant="outline" className="text-xs font-normal">
                    {career}
                  </Badge>
                ))}
              </div>
          ...
          <Card className="border-border/60">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-foreground">관련 학과</h3>
              </div>
              <div className="flex flex-wrap gap-1.5">
          ...
          <CardContent className="p-4">
            ...
            {isAvailable ? (
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {schoolData.schoolName}에서는 현재 개설되지 않은 과목입니다
              </div>
            )}
          </CardContent>
        </Card>
