...
function buildRecommendedNames(
  ...
  getProfessionalSubjectsForTags(interests, catalog.getSubjectByName).forEach((s) =>
    names.add(s.name),
  );
  return names;
}
...
function RoadmapContent() {
  ...
  const handleExportImage = useCallback(() => {
    ...
    try {
      ...
      ctx.font = `400 11px ${FONT}`;
      ctx.fillStyle = "#d1d5db";
      const wm = `${schoolData.schoolName} · 커리컴퍼스`;
    ...
    <div className="min-h-dvh pb-safe">
      ...
      {toastMsg && (
        <div
          ...
          aria-live="polite"
          aria-atomic="true"
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 rounded-full bg-foreground px-5 py-2.5 text-sm font-medium text-background shadow-lg"
        >
          {toastMsg}
        </div>
      )}
